...
// 0.3.0 起，本轮交付把 docs/09 原先按 phase-2..phase-5 分批交付的全部邮件能力
// （accounts/identities、folders、search/recent、message 读写、attachments、draft
// 含 send、operations、bounded watch；不含 calendar）合并冻结进同一轮契约与传输/
// Experiment 特权桥实现。`phase` 字段保留用于对照历史设计文档的分阶段表述，
// 不再是本轮命令是否被冻结/实现的门禁维度——真正的门禁是 extension/src/protocol.ts
// 与 extension/bridge/api.js 里按 route 静态声明的 risk/capability，未授予对应
// capability（尤其是 message delete 的 mail.delete-confirmed.v1、watch 的
// mail.watch.v1）时一律 E_POLICY_DENIED 失败关闭。
export const COMMANDS: readonly CommandSpec[] = [
  ...
  { path: ["message", "delete"], summary: "永久删除邮件；--prepare 生成预览、--confirm 提交绑定 Thunderbird UI 人工确认回执的确认，不接受 --force/--yes", risk: "destructive", phase: "phase-3" },
  ...
  { path: ["operations", "get"], summary: "查询可逆/外发/破坏性操作的当前状态", risk: "read", phase: "phase-2" },
  { path: ["calendar", "list"], summary: "列出日历", risk: "read", phase: "phase-3" },
  { path: ["calendar", "events"], summary: "查询日历事件", risk: "read", phase: "phase-3" },
  { path: ["watch"], summary: "以 --jsonl 输出有限时长的邮件事件流（CLI 侧轮询，非 server）", risk: "read", phase: "phase-3" },
] as const;
